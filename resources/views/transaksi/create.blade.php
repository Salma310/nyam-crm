...
<body>
    ...
    <form action="{{ url('transaksi/add') }}" method="POST" id="form-tambah">
        @csrf
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Agen</label>
                        <select name="agen_id" id="agen_id" class="form-control" required>
                            <option value="">Pilih Agen</option>
                            @foreach ($agen as $a)
                                <option value="{{ $a->agen_id }}">{{ $a->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <div class="item-row">
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label>Barang</label>
                                    <select name="barang[0][barang_id]]" class="form-control barang-select"
                                ...
                                <div class="form-group col-md-2">
                                    <label>Harga</label>
                                    <input type="number" name="barang[0][harga]" class="form-control harga-input"
                                        readonly>
                                </div>
                                <div class="form-group col-md-2">
                                    <label>Qty</label>
...
<script>
    ...
    $(document).on('change', '.barang-select', function() {
        ...
        const hargaInput = $(this).closest('.item-row').find('.harga-input');

        if (barangId && agenId) {
            $.get(`/harga-agen/${agenId}/${barangId}`, function(response) {
                if (response && response.harga) {
